    ...
    class Game
    {
        ...
        public void DoTurn(int Team)
        {
            if (IsPlayersTurn)
            {
                ...
                {
                    if (SetGameStateCellValue(int.Parse(UserChoice.ToString()), Team)) IsPlayersTurn = false;
                }
            }
            else
            {
                int EnemyTeam = GetEnemyTeam(Team);
                SetGameStateCellValue(AI.FindBestMove(GameState, EnemyTeam), EnemyTeam);
                IsPlayersTurn = true; // if for some reason the computer tries to make an illegal move, we'll just skip the computers turn.
            }
        }
        public int GetTeam()
        {
        ...
        bool SetGameStateCellValue(int Position, int CellValue)
        {
